...
# Modify by Sudipta Sarkar on date 10 February 2025 for finetuning on video classification tasks using imagenet pretrained hiera.
...
from Hiera.sifar_hiera import hiera_small_224, hiera_base_224
...
def get_args_parser():
    ...
    # model specific
    parser.add_argument('--model', default='hiera_small', type=str, help='Name of model to train')
    ...
    parser.add_argument('--hiera_model_path', type=str, default="", help='Imagenet pretrained hiera model path for loading weights')
    return parser
...
def main(args):
    ...
    if args.model == 'hiera_small':
        model = hiera_small_224(
            num_classes=args.class_numbers, pretrained = True, 
            strict = False, duration = args.duration, 
            super_img_rows = args.super_img_rows,
            input_size = (args.input_size * args.super_img_rows,args.input_size * args.super_img_rows)
        )
    elif args.model == 'hiera_base':
    ...
    """
    #  This section is need whenever compute node have no access to internet and we need to load imagenet pretrained hiera weights from local path. If the compute node have access to internet, then the pretrained weights will be automatically downloaded and loaded by the model creation code above, and this section can be safely ignored.

    if args.hiera_model_path:
        print("Loading Imagenet pretrained Hiera weights from :",args.hiera_model_path)
        ckpt = torch.load(args.hiera_model_path,  map_location="cpu", weights_only=True)
    ...
    train_augmentor = get_augmentor(True, args.input_size, mean, std, threed_data=args.threed_data,
                                    version=args.augmentor_ver, scale_range=args.scale_range, dataset=args.dataset, randomrotation=args.randomrotation)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Hiera training and evaluation script', parents=[get_args_parser()])
