...
from video_transforms import (GroupRandomHorizontalFlip, GroupOverSample,
                               ...
                               GroupNormalize, Stack, ToTorchFormatTensor, GroupRandomScale, GroupColorJitter, GroupRandomRotation, GroupRandAugment)
...
def get_augmentor(is_train: bool, image_size: int, mean: List[float] = None,
                  ...
                  modality: str = 'rgb', num_clips: int = 1, num_crops: int = 1, dataset: str = '', colorjitter: bool = False, randomrotation: int = 0, randomarg: bool = False):
    ...
    if modality == 'sound':
        ...
        if is_train:
            ...
            if colorjitter:
                #print("Color Jitter = ", colorjitter)
                augments += [GroupColorJitter(0.5,0.3,0)]
                #exit(0)

            if randomarg:
                #print("Group Random Argment = ", randomarg)
                augments += [GroupRandAugment(2,9,31)]
                #exit(0)
            if version == 'v1':
                augments += [
...
def build_dataflow(dataset, is_train, batch_size, workers=36, is_distributed=False):
    ...
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle,
                                              num_workers=workers, pin_memory=True, sampler=sampler, prefetch_factor=4)
