...
def interpolate_pos_embed(state_dict, new_size):
    ...
    #print(new_size)
    #exit(0)  
    num_patches_new = int(new_size/4)    #168 #224 #168 #int(new_size / model.patch_embed.patch_size)  
...
def pretrained_model(checkpoints: Dict[str, str], default: str = None) -> Callable:
    ...
    def inner(model_func: Callable) -> Callable:
        def model_def(pretrained: bool = False, checkpoint: str = default, strict: bool = True, **kwdargs) -> nn.Module:
           ...
           #-----------------------------------------------------------------------------------------------#
            img_size = int(kwdargs.get("input_size")[0])
            #print(img_size)
            #exit(0)

            state_dict = interpolate_pos_embed(state_dict, img_size)   # or 672 or 896
